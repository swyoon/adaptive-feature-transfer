...
if [ "$use_downstream" = "True" ]; then
    image_dir="./artifacts/251027/hybrid/edm_aft_${aft_score}_hybrid_steering_with_downstream/flowers-${seed}"
else
    image_dir="./artifacts/251027/hybrid/edm_aft_${aft_score}_hybrid_steering/flowers-${seed}"
fi
...
# auto
if [ "$use_downstream" = "True" ]; then
    steer_method=edm_aft_${aft_score}_hybrid_steering_with_downstream
else
    steer_method=edm_aft_${aft_score}_hybrid_steering
fi
...
CUDA_VISIBLE_DEVICES=${device} python generate_edm_hybrid_steering.py \
    --seed ${seed} \
    --edm_ckpt ${edm_ckpt} \
