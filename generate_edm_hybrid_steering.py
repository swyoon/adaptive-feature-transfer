...
def main(seed, edm_ckpt, aft_module, aft_score, num_target_images, save_dir, class_names, use_downstream, gradient_scale, guide_every, no_steering):
    DEVICE = 'cuda'
    config = f"""
    ...
    for _ in tqdm(range(int(np.ceil(num_target_images / config['batch_size'])))):
        with torch.autocast(device_type=next(iter(generator.parameters())).device.type, dtype=torch.float16):
            result = generator.sample_hybrid_steering(fkd_args=FKD_ARGS, reward_fn=reward_fn if not no_steering else None, reward_fn_args=reward_fn_args, guidance_scale=gradient_scale, guide_every=guide_every)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--gradient_scale', type=float, default=1.0, help='Gradient scale for gradient steering')
    parser.add_argument('--guide_every', type=int, default=1, help='Frequency of gradient steering application')
    parser.add_argument('--no_steering', action='store_true', help='If set, do not use FKD steering (for ablation)')
    args = parser.parse_args()

    print("generating data with edm hybrid steering...")
    ...
    main(seed, edm_ckpt, aft_module, aft_score, num_target_images, save_dir, class_names, use_downstream, args.gradient_scale, args.guide_every, args.no_steering)