...
class EDM(nn.Module):
    ...
    def sample_hybrid_steering(
        ...
        for i, (t_cur, t_next) in list(enumerate(zip(t_steps[:-1], t_steps[1:]))):  # 0, ..., N-1
            ...
            # gradient guidance
            if reward_fn is not None and i % guide_every == 0:
                reward_fn_args["targets"] = class_labels.nonzero(as_tuple=True)[1].detach()
